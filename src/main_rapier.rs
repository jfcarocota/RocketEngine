...
impl World {
    ...
    fn step_physics(&mut self) {
        self.physics_pipeline.step(
            &Vector2::new(0.0, 98.1), // Gravity (stronger for pixels)
...
fn main() {
    let mut window = Window::new(
        "RocketEngine with Rapier2D Physics!",
        WIDTH,
        HEIGHT,
    ...
    println!("RocketEngine with Rapier2D Physics started!");
    println!("Use arrow keys to move the player sprite.");
    println!("Watch entities interact with realistic physics!");
    println!("Rapier2D provides: proper collision response, friction, restitution, and more!");
